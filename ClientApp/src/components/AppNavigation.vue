<template>
    <span>
    <!-- drawer start-->
        <v-navigation-drawer app v-model="drawer" class="white" light disable-resize-watcher>
            <v-list>
                <template>
                    <v-list-tile :key="homeItem.title">
                        <router-link :to="homeItem.to">
                            <v-list-tile-content>{{homeItem.title}}</v-list-tile-content>
                        </router-link>
                    </v-list-tile>
...
export default {
    ...
    data() {
        return {
            ...
            leftItems: [
                { title: 'Character', itemLinks: [{ itemTitle: 'Create Character', to: 'create-character' }]},
    ...
    computed: {
        items: function() {
            return [ this.homeItem ].concat(this.leftItems)
